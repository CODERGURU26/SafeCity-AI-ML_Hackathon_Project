...
export const getCrimeType = (description) => {
  for (const [type, descriptions] of Object.entries(crimeCategories)) {
    if (
      descriptions.some((d) =>
        description.toLowerCase().includes(d.toLowerCase()),
      )
    ) {
      return type;
    }
...
export const aggregateByTimeframe = (data, timeframe = "monthly") => {
  ...
  data.forEach((record) => {
    ...
    aggregated[timeKey].policeAllocated += parseInt(
      record["Police Deployed"] || 0,
    );

    const crimeType = getCrimeType(record["Crime Description"]);
    aggregated[timeKey].crimeTypes[crimeType] =
      (aggregated[timeKey].crimeTypes[crimeType] || 0) + 1;

    // Hourly pattern
    const hour = new Date(record["Time of Occurrence"]).getHours();
    aggregated[timeKey].hourly[hour] =
      (aggregated[timeKey].hourly[hour] || 0) + 1;
  });
...
export const aggregateByCrimeType = (data) => {
  ...
  data.forEach((record) => {
    ...
    crimeStats[crimeType].policeAllocated += parseInt(
      record["Police Deployed"] || 0,
    );
...
export const aggregateByLocation = (data) => {
  ...
  data.forEach((record) => {
    ...
    locationStats[city].policeAllocated += parseInt(
      record["Police Deployed"] || 0,
    );

    const crimeType = getCrimeType(record["Crime Description"]);
    locationStats[city].crimeTypes[crimeType] =
      (locationStats[city].crimeTypes[crimeType] || 0) + 1;
    ...
    const hour = new Date(record["Time of Occurrence"]).getHours();
    locationStats[city].hourly[hour] =
      (locationStats[city].hourly[hour] || 0) + 1;
  });
...
export const generatePredictiveInsights = (data, location = null) => {
  ...
  const closedCases = filteredData.filter(
    (d) => d["Case Closed"] === "Yes",
  ).length;
  ...
  const insights = [
    ...
    },
    {
      ...
      description:
        "New crime cluster detected near Kurla station - recommend increased patrolling",
      confidence: "78%",
      icon: "MapPin",
    },
    {
...
export const calculateStats = (data) => {
  ...
  const totalPolice = data.reduce(
    (sum, d) => sum + parseInt(d["Police Deployed"] || 0),
    0,
  );
